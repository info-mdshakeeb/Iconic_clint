...
const PrimaryButton = ({ children, px, py }) => {
    // console.log(button);
    return (
        <button className="relative group overflow-hidden px-4 rounded-full flex space-x-2 w-full h-full
        items-center bg-gradient-to-r from-gray-700 via-gray-900 to-black hover:to-blue-700">
            <span className={`relative text-sm font-bold text-white px-${px} py-${py}`}>{children}</span>
